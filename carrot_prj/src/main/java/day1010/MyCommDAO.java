...
public class MyCommDAO {
	...
	public List<MyCommVO> selectMC(String id) throws SQLException {
		...
		//3. 쿼리문 생성객체 얻기
			...
			select
			.append("	select	id, product_idx, comment_idx, reply_idx, title, contents, posted_date	")
			.append("	from	product_comment																")
			.append("	where	id = ?, deleted = N															");
	...
	public int updateMC(MyCommVO mcVO) throws SQLException {
		...
		//1. 드라이버로딩
				...
				//3. 쿼리문 생성객체 얻기
					...
					.append("	set			contents = re_contents, deleted = Y								")
					.append("	where		id = ?, product_idx = ?, comment_idx = ?, reply_idx = ?		");
					
					pstmt=con.prepareStatement(updateCpEmp.toString());
				//4. 바인드 변수에 값 설정
					pstmt.setString(1, mcVO.getId());
					pstmt.setInt(2, mcVO.getProduct_idx());
					pstmt.setInt(3, mcVO.getComment_idx());
					pstmt.setInt(4, mcVO.getReply_idx());
				//5. 쿼리문 수행 후 결과 얻기
