...
public class MySalesDAO {
	 ...
	 * @throws SQLException
	 */
	public List<MySalesVO> selectSale(MySalesVO msVO) throws SQLException{
		...
		//3. 쿼리문 생성객체 얻기
			...
			selectAll
			.append("select	p.thumbnail, p.title, lc.gu_idx, lc.gu, p.post_date, p.reserv, p.sold_chk, p.price, p.comment_cnt, p.like_cnt	")
			.append("from	product p, loc_category lc																								")
			.append("where	id = ?	and sold_chk = N 																									");
			
			pstmt = con.prepareStatement(selectAll.toString());
		//4. 바인드 변수 설정
			pstmt.setString(1, msVO.getId());
			pstmt.setString(2, msVO.getSold_chk());
		//5. 쿼리문 생성 후 결과 얻기
			...
			while( rs.next() ) {
				...
				mssVO.setReserv(rs.getString("reserv"));
	...
	public int updateResev(MySalesVO msVO) throws SQLException{
		int updateCnt = 0;
		...
		//4. 바인드 변수에 값 설정
			pstmt.setString(1, msVO.getReserv());
	...
	public List<MySalesVO> selectTrader(String id) throws SQLException {
		...
		//3. 쿼리문 생성객체 얻기
			...
			.append("from product p, loc_category lc																")
			.append("where	id = ?	and sold_chk = Y 																");
